...
class Ventana(Frame):
	...
	def animacion(self): 
		...
		self.obtener_tiempo()
		gif = Image.open('weather/buscar.gif')
		frames = gif.n_frames
		if self.click == True:			
			for i in range(1, frames):				
				self.inicio = PhotoImage(file ='weather/buscar.gif', format='gif -index %i' %(i))
	...
	def obtener_tiempo(self):
	    ...
	    #key :  '48a3dd691689b68e49d4c40f21b447a8'
		#API = 'api.openweathermap.org/data/2.5/weather?q={city name}&appid={API key}'

		#Actualizar  la API key con la de su cuenta: 
	    API = 'https://api.openweathermap.org/data/2.5/weather?q=' +ciudad+ '&appid=48a3dd691689b68e49d4c40f21b447a8'  
	    try:
	...
	def widgets(self):
		self.inicio = PhotoImage(file ='weather/buscar.gif')
		self.imagen_temp = PhotoImage(file ='weather/temperatura.png')
		self.imagen_temp_min = PhotoImage(file ='weather/temp_min.png')
		self.imagen_temp_max = PhotoImage(file ='weather/temp_max.png')
		self.imagen_humedad = PhotoImage(file ='weather/humedad.png')
		self.imagen_viento = PhotoImage(file ='weather/viento.png')
		self.imagen_presion = PhotoImage(file ='weather/presion.png')
...
if __name__ == "__main__":
	...
	ventana.call('wm', 'iconphoto', ventana._w, PhotoImage(file='weather/temperatura.png'))
